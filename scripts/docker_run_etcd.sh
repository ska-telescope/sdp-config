...
echo  $ docker run --link etcd --env ETCDCTL_ENDPOINTS=etcd:2379 --env ETCDCTL_API=3 -i quay.io/coreos/etcd:${version} /usr/local/bin/etcdctl endpoint health
...
echo  $ SDP_CONFIG_HOST=$SDP_CONFIG_HOST SDP_CONFIG_PORT=$SDP_CONFIG_PORT ska-sdp ...
echo
